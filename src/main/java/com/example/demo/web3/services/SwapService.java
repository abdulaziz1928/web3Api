...
public class SwapService {
        ...
        // private static final String factoryContractAddress =
        ...
        public String swapExactEthForToken(JSONObject input) throws Exception {
                ...
                StaticGasProvider s = new StaticGasProvider(
                                def.getGasPrice().add(def.getGasPrice().divide(new BigInteger("10"))),
                                def.getGasLimit());
                UniswapV2Router02 contractRouter = UniswapV2Router02.load(routerContractAddress, client, credentials,
                                s);

                List amountsout = contractRouter
                                .getAmountsOut(new BigInteger(amnt),
                                                Arrays.asList(contractRouter.WETH().send(), token1))
                                .send();
                ...
                return contractRouter.swapExactETHForTokens(amountOutMin,
                                Arrays.asList(contractRouter.WETH().send(), token1),
                                credentials.getAddress(),
                                BigInteger.valueOf((System.currentTimeMillis() / 1000) + 60 * 10), amount)
        ...
        public JSONObject getPrice(JSONObject input) throws Exception {
                ...
                Tuple5<BigInteger, BigInteger, BigInteger, BigInteger, BigInteger> s = contract.latestRoundData()
                                .send();
        ...
        public String swapExactTokensForETH(JSONObject input) throws Exception {
                ...
                UniswapV2Router02 contractRouter = UniswapV2Router02.load(routerContractAddress, client, credentials,
                                s);
                List amountsout = contractRouter.getAmountsOut(new BigInteger(amnt),
                                Arrays.asList(token0, contractRouter.WETH().send())).send();
                double samountOutMin = Double.parseDouble(amountsout.get(1).toString()) * 90 / 100;
                long m = ((long) samountOutMin);
                BigInteger amountOutMin = new BigInteger(String.valueOf(m));
                System.out.println(new BigInteger(String.valueOf(m)));
                ...
                return contractRouter
                                .swapExactTokensForETH(amount, amountOutMin,
                                                Arrays.asList(token0, contractRouter.WETH().send()),
                                                credentials.getAddress(),
        ...
        public String swapExactTokensForTokens(JSONObject input) throws Exception {
                ...
                UniswapV2Router02 contractRouter = UniswapV2Router02.load(routerContractAddress, client, credentials,
                                s);
                List amountsout = contractRouter.getAmountsOut(new BigInteger(amnt), Arrays.asList(token0, token1))
                                .send();
                double samountOutMin = Double.parseDouble(amountsout.get(1).toString()) * 90 / 100;
                long m = ((long) samountOutMin);
                BigInteger amountOutMin = new BigInteger(String.valueOf(m));
                ...
                return contractRouter
                                .swapExactTokensForTokens(amount, amountOutMin, Arrays.asList(token0, token1),
                                                credentials.getAddress(),
                                                BigInteger.valueOf((System.currentTimeMillis() / 1000) + 60 * 10))
                                .sendAsync().get().getTransactionHash();
        }
